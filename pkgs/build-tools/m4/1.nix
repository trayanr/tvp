{ pkgs, tvp }:
{
  "1.4.17" = {
    builder = ./build-1.4.19.nix;
    status = {
      level = "broken";
      reason = "The bundled gnulib tests SIGSTKSZ in a preprocessor conditional (lib/c-stack.c), and glibc 2.34 redefined it from a constant to a sysconf() call, so the file no longer preprocesses.";
      needs = "A period glibc (< 2.34). The code was correct when released; the substrate moved under it. Upstream's own fix was a full gnulib rewrite that arrived in 1.4.19, not a change TVP can apply backwards.";
    };
    sha256 = "sha256-POclEz7lUri0usp4N/t3KUCyXoGyqdySU3rq9zNTjJ4=";
    deps = { };
  };

  "1.4.18" = {
    builder = ./build-1.4.19.nix;
    status = {
      level = "broken";
      reason = "The bundled gnulib tests SIGSTKSZ in a preprocessor conditional (lib/c-stack.c), and glibc 2.34 redefined it from a constant to a sysconf() call, so the file no longer preprocesses.";
      needs = "A period glibc (< 2.34). The code was correct when released; the substrate moved under it. Upstream's own fix was a full gnulib rewrite that arrived in 1.4.19, not a change TVP can apply backwards.";
    };
    sha256 = "sha256-qyYzkhpc045IeXv1UhrSWb3EuXkHgDSjt5DX/sVJP6s=";
    deps = { };
  };

  "1.4.19" = {
    builder = ./build-1.4.19.nix;
    sha256 = "sha256-O+SibYJf/f2lKlb8QyRkVpiaNjAJPM7T+930dx7linA=";
    deps = { };
  };

  "1.4.20" = {
    builder = ./build-1.4.19.nix;
    sha256 = "sha256-asT8Mc5EDevmOYfC67+de2Y05np8Mnklfcc2Hei9s+8=";
    deps = { };
  };

  "1.4.21" = {
    builder = ./build-1.4.19.nix;
    sha256 = "sha256-OK5Z96ML+cEIGTzFwl+7BgFPIeIwx+3i7/YU97fDftg=";
    deps = { };
  };
}
